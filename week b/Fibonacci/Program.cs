...
namespace Fibonacci
{
    class Program
    {
        ...
        static List<int> memoizedValues = new List<int>();
        ...
        static int FibonacciIterative(int i)
        {
            ...
            // return the ith number in fibonacci sequence
            for (int j = 1; j < i; j++) // do ith times
            {
                sum =+ sequence[j] + sequence[j - 1];
                sequence.Add(sum);  //add fib number to sequence
            }
            return sum;
        }
