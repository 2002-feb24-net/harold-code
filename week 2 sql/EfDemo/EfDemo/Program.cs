    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            using (var context = new PersonContext())
            {
                ...
                string newName = InputModifyName(person);
                person.Name = newName;
            ...
            Console.WriteLine("Name of person to add: ");
            string name = Console.ReadLine();
            Console.WriteLine($"{name}'s ID: ");
            int nameId = Convert.ToInt32(Console.ReadLine());
            AddPerson(nameId, name);

            // prompt for the name of a person to delete. (call your method)
            Console.WriteLine("Name of person to delete: ");
            string nameToDelete = Console.ReadLine();
            DeletePersonByName(nameToDelete);

        }

        public static string InputModifyName(Person person)
        {
            Console.WriteLine($"Would you like to modify {person.Name}'s name?");
